...
// Header copy for the Self-Mastery dashboard. Marriage/Dynamics/Scripts
// content lives in the Relationship Hub's "Relationship Practice" tab.
const headerCopy = {
  ...
  const renderIntegration = () => (
    <div>
      ...
      <Card className="mt-6">
        ...
        <div className="mt-5 rounded-2xl border border-emerald-400/20 bg-emerald-400/10 p-4 text-sm leading-6 text-emerald-100">
          If any one piece is missing — the energy is denied, the discipline is absent, or the connection is fear-based — the other two usually start to distort too. Use the Energy, Self-Mastery, and Integration tabs as a loop, not a one-time read.
        </div>
      </Card>
    </div>
  );
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div>
              {onNavigate && (
                ...
                >
                  Looking for relationship content? → Relationship Hub
                </button>
              )}
              <div className="mb-4 flex flex-wrap gap-2">
                {headerCopy.badges.map(([tone, label]) => (
